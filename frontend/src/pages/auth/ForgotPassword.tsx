...
export function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('loading');
    try {
      const res = await apiCall<{ message: string }>('/api/auth/forgot-password', {
        method: 'POST',
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-center mt-4">
                <Link to="/login" className="flex items-center text-xs text-[#A3A3A3] hover:text-white transition-colors">
